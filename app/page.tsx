...
import { Box, CarFront, Cog, DollarSign, Download, Droplets, Gauge, Layers, Mail, MapPin, Phone, Printer, ScanSearch, ShieldCheck, Sofa, Star, SwitchCamera, Wrench } from 'lucide-react';
...
function ScannerView({ intakeData, onBack }: { intakeData: IntakeData; onBack: () => void }) {
  ...
  const generateMockSTL = () => {
    const blob = new Blob([`solid mechanicure_mock_part\nfacet normal 0 0 1\n outer loop\n  vertex 0 0 0\n  vertex 20 0 0\n  vertex 0 20 0\n endloop\nendfacet\nendsolid`], { type: 'model/stl' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'mechanicure_replacement_part.stl'; a.click(); URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black font-mono text-white">
      ...
      <div className="relative h-screen w-full">
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 z-20 h-52 w-52 -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/70 shadow-[0_0_30px_rgba(255,255,255,0.2)]" />

        <button
          type="button"
          onClick={() => setFacingMode((prev) => (prev === 'environment' ? 'user' : 'environment'))}
          className="absolute bottom-6 left-1/2 z-30 -translate-x-1/2 rounded-full border border-white/30 bg-black/50 p-4 shadow-lg backdrop-blur"
        >
            ...
            {!loading && diagnosis && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-cyan-300/35 bg-slate-900/70 p-4 backdrop-blur-md">
                ...
                <div className="flex flex-wrap gap-3">
                  <button onClick={generateMockSTL} className="inline-flex items-center gap-2 rounded-md border border-cyan-300/45 px-4 py-2 text-xs"><Download className="h-4 w-4" /><Box className="h-4 w-4" />Download .STL File</button>
